...
export class RutaService {
  ...
  getRutas(): Observable<Ruta[]> {
    return this.http.get<Ruta[]>(this.apiUrl, { headers: this.headersEmpresa() });
  }

  getRuta(id: number): Observable<Ruta> {
    return this.http.get<Ruta>(`${this.apiUrl}/${id}`, { headers: this.headersEmpresa() });
  }
  ...
  eliminarRuta(id: number): Observable<void> {
    return this.http.delete<void>(`${this.apiUrl}/${id}`, { headers: this.headersEmpresa() });
  }

  filtrarPorEstado(estado: string): Observable<Ruta[]> {
    return this.http.get<Ruta[]>(`${this.apiUrl}/estado/${estado}`, { headers: this.headersEmpresa() });
  }

  filtrarPorTransportista(nombre: string): Observable<Ruta[]> {
    return this.http.get<Ruta[]>(`${this.apiUrl}/transportista/${nombre}`, { headers: this.headersEmpresa() });
  }

  filtrarPorFecha(fecha: string): Observable<Ruta[]> {
    return this.http.get<Ruta[]>(`${this.apiUrl}/fecha/${fecha}`, { headers: this.headersEmpresa() });
  }

  cerrarRuta(id: number): Observable<Ruta> {
    return this.http.put<Ruta>(`${this.apiUrl}/cerrar/${id}`, {}, { headers: this.headersEmpresa() });
  }
